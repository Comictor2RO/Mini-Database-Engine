...
GUI::GUI(Engine &engine, const Config &config)
    : engine(engine), server(engine, config.port, config.aux_max_failures, config.aux_timeout, config.bypass_localhost, config.thread_count), serverRunning(false), isDark(true), cursorPos(0), backspaceRepeatTimer(0)
{}
